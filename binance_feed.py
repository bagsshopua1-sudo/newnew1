...
равно следует за Binance через арбитраж - значит настоящая структура книги
(крупные заявки, wall advantage) честнее видна там, где ликвидность
глубокая, то есть на Binance. Исполнение (лимитки, 0 комиссий) остаётся на
Lighter - см. bot.py: сигнал берёт направление и уровень с Binance, но цену
...
ПРИМЕЧАНИЕ (21.08, полная перестройка торговой логики): раньше в этом же
соединении также шла подписка на aggTrade (поток исполненных сделок,
binance_trades.py) - новая механика входа/выхода (см. signals.py) построена
только на displayed-размере заявок в книге (wall advantage ratio), реальный
исполненный объём ей больше не нужен, поэтому aggTrade-подписка убрана -
этот файл снова отвечает только за стакан, как и было изначально.
"""
import asyncio
import json
import logging
