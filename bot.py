...
async def run_trading():
    ...
    async def _maybe_signal(signal_snap, lighter_snap):
        ...
        # По прямой просьбе пользователя 18.08 (вечер): сам алгоритм детекции
        # (стенки/дисбаланс/ABSORPTION/BREAKOUT в signals.py) НЕ трогаем -
        # разворачиваем только итоговую сторону входа на противоположную.
        # Основание: разбор 60 сделок за последние ~90 минут (после фикса
        # кулдауна/wall_dominance) показал минус на ОБЕИХ сторонах примерно
        # поровну (SHORT -8.60 на 33 сделках, LONG -4.25 на 27) - это не
        # перепутанный отдельный кусок логики на одной стороне, а системная
        # штука на обеих сразу, но раз пользователь просит именно зеркалку -
        # делаем её в ОДНОЙ точке (a не в 20 местах по всему order_manager.py),
        # чтобы SL/TP, трейлинг и "умный" выход (_thesis_invalidated - см.
        # order_manager.py, все ветки уже читают pos.side симметрично, ничего
        # там менять не нужно) продолжали работать ровно как раньше, просто
        # относительно развёрнутой стороны.
        signal.side = "short" if signal.side == "long" else "long"
