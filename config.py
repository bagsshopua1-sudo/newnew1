...
class Config:
    ...
    wall_backup_range_pct: float = field(default_factory=lambda: _f("WALL_BACKUP_RANGE_PCT", 0.3))
    # Подложка должна быть не меньше этой доли от объёма самой стенки.
    wall_backup_min_ratio: float = field(default_factory=lambda: _f("WALL_BACKUP_MIN_RATIO", 0.7))
