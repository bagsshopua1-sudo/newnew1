...
class Config:
    ...
    # НАЙДЕНО В ПРОДЕ 19.08: Render free tier периодически перезапускает процесс
    # сам по себе (без деплоя) - раз в ~10-90 минут (см. историю "Бот запущен" в
    # логах), и это НЕ безобидно. Живой пример: старый процесс (instance tnhqq)
    # ещё держал открытую SHORT-позицию (64462.12), когда параллельно поднялся
    # новый процесс (instance lf59d) и, пока оба несколько секунд работали
    # ОДНОВРЕМЕННО с независимым состоянием, новый процесс уже за 0.3-3 секунды
    # после подключения к Binance WS успел выдать ABSORPTION-сигнал и открыть
    # СВОЮ отдельную позицию - на полностью свежем/неполном wall-tracking
    # (self.tracked только начал наполняться, книга могла быть не до конца
    # синхронизирована после реконнекта). Отсюда и ощущение "открывает рандомно" -
    # это буквально другой процесс с пустой историей, а не тот же самый бот с
    # каким-то глюком в логике. Добавляем стартовую паузу - НЕ обрабатывать
    # сигналы первые STARTUP_GRACE_SEC секунд после запуска, пока стакан/
    # wall-tracking не устаканятся. Старую "потерянную" при рестарте позицию
    # это не восстановит (paper-состояние живёт только в памяти процесса - см.
    # docstring bot.py), но это отдельный, более крупный вопрос архитектуры
    # (нужна персистентность вне процесса), не решается одной строкой.
    ...
    #
    # БОЛЬШЕ НЕ ИСПОЛЬЗУЕТСЯ напрямую (оставлено для истории/если понадобится
    # откатить) - 18.08 вечером по прямой просьбе пользователя заменено на
    # пару WALL_FLIP_SHRINK_RATIO + сам порог "стенка калибра входа" ниже -
    # ratio=1.0 (любой перевес, хоть на цент) оказался самым шумным триггером
    # из всех, см. комментарий там.
    wall_dominance_ratio: float = field(default_factory=lambda: _f("WALL_DOMINANCE_RATIO", 1.0))
    # === Разворот доминирования стенок - решительная версия (18.08 вечером) ===
    # Прямая просьба пользователя, дословный пример: "если он лонгует и
    # заявка с 1.5кк [размер стенки на входе] стала 400к, а верхняя в шорт
    # стала 1.5кк - то надо выходить". Не "чуть перевесило" (это и было
    # проблемой в WALL_DOMINANCE_RATIO=1.0 выше), а решительный разворот:
    # держащая стенка просела до этой доли от своего же размера на входе...
    wall_flip_shrink_ratio: float = field(default_factory=lambda: _f("WALL_FLIP_SHRINK_RATIO", 0.3))
    # ...И встречная стенка успела дорасти до калибра "настоящей" стенки -
    # используем ТОТ ЖЕ порог, что даёт сигнал на вход (BINANCE_WALL_MIN_USD/
    # WALL_MIN_USD, см. ниже) - отдельного числа под это не заводим, порог
    # "что считается реальной стенкой" один и тот же что на входе, что на
    # развороте. См. использование в _thesis_invalidated (order_manager.py).
    ...
    # НАЙДЕНО В ПРОДЕ 19.08 - настоящая причина, почему серия dominance почти
    # никогда даже НАЧИНАЛАСЬ (не только сбрасывалась - см. коммент у
    # pos.wall_dominance_streak в order_manager.py). snap.ask_walls/bid_walls,
    # которые читает _thesis_invalidated, это СЫРОЙ снепшот Binance partial-
    # depth стрима (см. BINANCE_WS_DEPTH_LEVELS=20 верхних уровней книги,
    # binance_feed.py/market_data.analyze_book) - НЕ персистентный трекинг,
    # как self.tracked в signals.py. На плотном стакане реальная крупная
    # стенка легко выпадает за пределы верхних 20 уровней на ОДНОМ конкретном
    # тике, хотя сама заявка на бирже никуда не делась - в этот момент
    # blocking_walls/holding_walls приходят просто ПУСТЫМИ и считаются как 0.
    # Живой пример из жалобы пользователя: SHORT 64463.70 - за все 34с сделки
    # не было ни единой строки "dominance:" вообще, хотя пользователь застал
    # явный флип 2М->500К встречная 2М - потому что на каждом тике, когда
    # проверка успевала сработать, snap.ask_walls/bid_walls были пустыми.
    # Держим последний РЕАЛЬНО увиденный размер стенки по каждой стороне ещё
    # это время после того, как она пропала из сырого снепшота - если за это
    # время она не появится снова, считаем её действительно исчезнувшей (как
    # раньше). НЕ путать с WALL_DOMINANCE_CONFIRM_TICKS выше - тот считает
    # подряд идущие тики, где dominance_now уже True, этот - не даёт самому
    # dominance_now ложно упасть в False только из-за дыры в сыром снепшоте.
    wall_presence_grace_sec: float = field(default_factory=lambda: _f("WALL_PRESENCE_GRACE_SEC", 3.0))

    # === Не фейдить в чистом боковике без движения (19.08, прямой вопрос
    # пользователя "как торговать только на рывках, а не на боковике") ===
    # TrendFilter.allows_fade() уже блокирует ABSORPTION (фейд) ПРОТИВ явного
    # тренда (EMA fast/slow разошлись) - но при trend=="flat" (EMA почти
    # совпадают, ровно боковик) фейд СЕЙЧАС разрешён в ОБЕ стороны без каких-
    # либо ограничений - см. документированный живой случай в TrendState.
    # is_dead (BTC простоял в коридоре 0.04% шесть минут, бот открыл 8 сделок
    # подряд (фейды в обе стороны на один и тот же шум), 8 из 8 в минус).
    # Старый гейт под это (is_dead, по диапазону цены за скользящее окно)
    # пересчитывали и включали обратно 3 раза за 18.08 - каждый раз снова
    # выключали: либо резал реальные медленные сносы (диапазон 90с мал, а
    # снос растянут на 9 минут), либо пропускал резкий шум. Не гадаем в
    # четвёртый раз с тем же is_dead - берём другую метрику, которая уже
    # используется в проекте как референс на "нормальную" волатильность
    # (TIME_EXIT_VOL_REF_PCT=0.03, тот же trend_state.volatility_pct) -
    # требуем, чтобы РЕАЛЬНОЕ движение цены (скользящий std% последних
    # тиков) было хоть сколько-то заметным, а не просто "EMA не разошлись".
    # НЕ применяется к BREAKOUT - пробой стенки сам по себе уже означает
    # реальное движение, эта проверка касается только ABSORPTION-фейдов при
    # trend=="flat" (см. signals.py). Порог ниже - первая прикидка (треть от
    # TIME_EXIT_VOL_REF_PCT), НЕ откалиброван по реальной выборке, как и
    # DEAD_RANGE_MIN_PCT в своё время - решение о том, что считать "мёртвым",
    # правильно принимать по логам (reason=flat_low_volatility), а не на
    # глаз, это и есть урок из трёх циклов is_dead.
    absorption_flat_min_volatility_pct: float = field(
        default_factory=lambda: _f("ABSORPTION_FLAT_MIN_VOLATILITY_PCT", 0.01))
    ...
    # Снижено до 10 сек 19.08 по прямой просьбе пользователя ("кулдаун уменьши
    # на 10 сек") - к этому моменту reversal_signal уже отключён (см.
    # handle_signal), поэтому это единственный живой кулдаун, реально влияющий
    # на поведение бота (REVERSAL_COOLDOWN_SEC ниже сейчас не участвует в
    # выполнении, т.к. ветка разворота вообще не доходит до входа).
    ...
    # Было снижено с 10 до 1 сек 18.08 - см. тот же комментарий у
    # REENTRY_COOLDOWN_SEC выше (аналогичная история). Найдено в проде 18.08
    # поздно вечером: с 1 сек серия из 8 разворотов подряд за ~90 секунд
    # (SHORT-LONG-SHORT-LONG...), интервалы между ними 1-33 сек - почти все
    # проходили мимо такого короткого кулдауна. Итог -4.16 USD за полторы
    # минуты, средняя сделка держалась считанные секунды. ПЕРВЫЙ разворот
    # после открытия позиции по-прежнему мгновенный (см. handle_signal) -
    # это ограничивает только ПОВТОРНЫЕ развороты подряд. Поднято обратно до
    # 20 сек, как REENTRY_COOLDOWN_SEC.
    reversal_cooldown_sec: float = field(default_factory=lambda: _f("REVERSAL_COOLDOWN_SEC", 20.0))
